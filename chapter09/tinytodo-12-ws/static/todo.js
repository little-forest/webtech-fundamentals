...
function observeTodo() {
  ...
  const ws = new WebSocket(`${wsScheme}${window.location.host}/observe`); // <1>

  ws.onmessage = function(ev) { // <2>
    const message = JSON.parse(ev.data);

    // 受信したメッセージが妥当であるかをチェック
    if (!isObject(message)) {
      console.warn("Invalid data: " + message);
      return;
    }
 ...
 */
function updateTodoItem(todoItem) {                  // <5>
