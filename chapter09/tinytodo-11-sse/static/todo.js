...
function observeTodo() {
  const eventSource = new EventSource("/observe");   // <1>

  eventSource.addEventListener("initial", (e) => {   // <2>
  ...
  eventSource.addEventListener("add", (e) => {       // <3>
    const data = JSON.parse(e.data);
  ...
  eventSource.addEventListener("update", (e) => {    // <4>
    const data = JSON.parse(e.data);
    if (!isObject(data) || !isObject(data.todoItem)) {
      console.warn("Invalid data: " + e.data);
    }
